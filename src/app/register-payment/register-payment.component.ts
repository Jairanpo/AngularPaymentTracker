...
export class RegisterPaymentComponent implements OnInit {
  ...
  onSubmit() {
    let amount = Number(
      this.paymentRegistrationForm.controls.amount.value
    ).toFixed(2);
  ...
  numberOnly(event): boolean {
    var currentAmount = !this.paymentRegistrationForm.value.amount
      ? ''
      : this.paymentRegistrationForm.value.amount;
